...
| **feed_version_number** | [uint64](https://developers.google.com/protocol-buffers/docs/proto#scalar) | optional | This field contains a non-negative integer used to link the static GTFS zip file (via feed_info.txt feed_version field) and the GTFS-rt feed message. If this field is provided, then the data in this feed message should only be applied to the GTFS data with a matching feed_info.feed_version value. When a producer roll out a new GTFS data with new ids (trip_id, etc.) and a new feed_version value, it should provide GTFS-realtime feed messages that match both datasets for at least 3 calendar days to allow consumers time to process the new GTFS dataset.  |
