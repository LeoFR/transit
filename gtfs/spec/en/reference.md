...
|  feed_version_number | Optional | The **feed_version_number** field contains a non-negative integer, always increasing between each releases of a new GTFS dataset (but not necessarily consecutive). This value should match the GTFS-realtime feed_header.feed_version_number to identify which GTFS dataset is associated with which GTFS-realtime messages. |
