...
|  feed_version_number | Optional | The **feed_version_number** field contains a non-negative integer, always increasing between each releases of a new GTFS dataset (but not necessarily consecutive). It would be used by the GTFS-rt API to specify which GTFS is currently used by the consumer, and therefore which set of real-time ids should be used in the GTFS-rt feeds. |
